<div class="form-group {{ $detail['cssClass'] }}">
    ...
    <select class="form-control form-select form-select-sm @error($colname) parsley-error @enderror"
        ...
        @foreach ($detail['data'] as $id => $val)
            <option value="{{ $id }}"
                {{ (isset($row) && $row->status == $id) || old($colname) ==  $id || (!isset($row) && $id == 1 ) ? 'selected' : '' }}>
                {{ $val }}
            </option>
        @endforeach
    </select>
